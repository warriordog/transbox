...
const getLData = (LBUser: string) => {
    return axios.get(`https://api.listenbrainz.org/1/user/${LBUser}/listens?count=1`).then((data) => {
      ...
      const id: string = data.data.payload.listens[0].track_metadata.mbid_mapping && data.data.payload.listens[0].track_metadata.mbid_mapping.recording_mbid ? `https://musicbrainz.org/recording/${data.data.payload.listens[0].track_metadata.mbid_mapping.recording_mbid}` : '#';
      return [title, img, artist, lastlisten, id];
    });
};
